...
export const createAdminRemoteFunction = form(registerSchema, async ({ name, email, password }) => {
  ...
  const { error, data } = await tryCatch(internalAdapter.createUser({
    email,
    name,
    password,
  ...
  const { error: linkAccountError, data: linkAccountData } = await tryCatch(internalAdapter.linkAccount({
