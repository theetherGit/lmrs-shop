...
export const productionLedger = sqliteTable("production_ledger",
  {
...
export const salesPerformance = sqliteTable("sales_performance",
  {
    id: text("id").primaryKey().$defaultFn(() => ulid()),
    // Date of sale
    ...
    appliedWeights: text('applied_weights', { 'mode': 'json' }).$type<Record<string, number>>().notNull(),
    // miscellaneous
...
export const factorType = sqliteTable('factor_type',
  {
    id: text('id').primaryKey().$defaultFn(ulid),
...
export const factorWeight = sqliteTable('factor_weight',
  {
    id: text("id").primaryKey().$defaultFn(ulid),
    // Type = weightTypes.factorSlug
    factorSlug: text("factor_slug").unique().notNull().references(() => factorType.factorSlug),
  ...
  (table) => [
    index('factor_weight_factor_slug_idx').on(table.factorSlug)
  ]
)
...
export const weightAuditLog = sqliteTable('weight_audit_log', {
  ...
  factorSlug: text("factor_slug").notNull().references(() => factorType.factorSlug),
...
export const forecast = sqliteTable('forecast',
  ...
  (table) => [
    index('forecast_date_idx').on(table.date)
  ]
)


export const datesheet = sqliteTable('datesheet',
  {
    id: text("id").primaryKey().$defaultFn(ulid),
    date: text('date').notNull(),
  ...
  (table) => [
    index('datesheet_date_idx').on(table.date)
  ]
)
...
export const holiday = sqliteTable('holiday',
  {
    id: text("id").primaryKey().$defaultFn(ulid),
    date: text('date').notNull(),
    type: text('type', { enum: ['government', 'mid_sem_break', 'national_holiday', 'weekend'] }).notNull(),
    reason: text('reason')
  },
  (table) => [
...
export const appSettings = sqliteTable('app_settings',
  ...
  (table) => [
    index('app_settings_date_idx').on(table.date)
  ]);
